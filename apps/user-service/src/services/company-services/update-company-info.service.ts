    ...
    async updateCompanyInfo(updateCompanyInfoDTO: UpdateCompanyInfoDTO, companyId: string): Promise<{ message: string, company: CompanyResponseDTO }> {
        try {
            ...
            if(updateCompanyInfoDTO.benefits) {
                const existingBenefits = await this.benefitRepository.findBy({ companies: { id: companyId } });
                const newBenefits = updateCompanyInfoDTO.benefits.map((benefit) => this.benefitRepository.create(benefit));
                company.benefits = [...existingBenefits, ...newBenefits];
                await this.benefitRepository.save(newBenefits);
            }

            if(updateCompanyInfoDTO.values) {
                const existingValues = await this.valueRepository.findBy({ companies: { id: companyId } });
                const newValues = updateCompanyInfoDTO.values.map((value) => this.valueRepository.create(value));
                company.values = [...existingValues, ...newValues];
                await this.valueRepository.save(newValues);
            }

            if(updateCompanyInfoDTO.jobs) {
                const existingJobs = await this.jobRepository.findBy({ company: { id: companyId } });
                const newJobs = updateCompanyInfoDTO.jobs.map((job) => this.jobRepository.create(job));
                company.openPositions = [...existingJobs, ...newJobs];
                await this.jobRepository.save(newJobs);
            }

            if(updateCompanyInfoDTO.careerScopes) {
                const existingCareerScopes = await this.careerScopeRepository.findBy({ companies: { id: companyId } });
                const newCareerScopes = updateCompanyInfoDTO.careerScopes.map((cs) => this.careerScopeRepository.create(cs));
            ...
            if (updateCompanyInfoDTO.socials) {
                const existingSocials = await this.socialRepository.findBy({ company: { id: companyId } });
                const newSocials = updateCompanyInfoDTO.socials.map(social => this.socialRepository.create(social));
