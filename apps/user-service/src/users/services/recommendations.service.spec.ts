...
describe('RecommendationsService', () => {
  ...
  it('covers recommendation input helpers and block lookup', async () => {
    ...
    expect(clampRecoLimit(2.9)).toBe(2);
    expect(vectorCentroid([])).toBeNull();
    expect(vectorCentroid([[0, 0]])).toBeNull();
    expect(
      vectorCentroid([
        [1, -1],
        [-1, 1],
    ...
    expect(vectorCentroid([[3, 4], [3, 4], [1]])).toEqual([0.6, 0.8]);
    expect(toVectorLiteral([1, 2])).toBe('[1,2]');
    expect(normalizeDegree('Master of Science')).toBeGreaterThan(
      normalizeDegree('Bachelor degree'),
    );
    expect(normalizeDegree('Associate diploma')).toBe(2);
    expect(normalizeDegree('High school certificate')).toBe(1);
    expect(normalizeDegree('Professional certificate')).toBe(0);
    expect(normalizeDegree('Doctor of Philosophy')).toBe(5);
    expect(normalizeDegree(null)).toBe(0);
    expect(extractYears('at least 4 years')).toBe(4);
    expect(extractYears('')).toBe(0);
    expect(extractYears('experienced')).toBe(0);
    expect(extractKeywords('The senior TypeScript platform engineer')).toEqual(
