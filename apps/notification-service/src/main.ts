...
async function bootstrap() {
  ...
  const metricsPort = configService.get<number>(
    'services.notification.metricsPort',
  );
  ...
  logger.log(
    `Notification service running (RPC TCP ${port}, metrics http://0.0.0.0:${metricsPort}/metrics)`,
  );
}
bootstrap();
