...
describe('MailerService', () => {
  ...
  async function createService() {
    ...
    const service = new MailerService(config as any, logger as any);
  ...
  it('rejects missing recipients before calling SMTP', async () => {
    const service = await createService();
    await expect(service.send({ subject: 'Invalid' } as any)).rejects.toThrow(
  ...
  it('fails construction immediately when required SMTP configuration is missing', () => {
    ...
    expect(() => new MailerService(config as any, logger as any)).toThrow(
      'Missing SMTP host',
    );
