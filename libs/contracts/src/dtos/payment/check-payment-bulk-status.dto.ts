...
export class CheckPaymentBulkStatusDTO {
  ...
  @IsString({ each: true })
  @Length(32, 32, { each: true, message: 'Each MD5 hash must be exactly 32 characters' })
  @Matches(/^[a-fA-F0-9]{32}$/, { each: true, message: 'Each MD5 hash must be a valid hexadecimal string' })
  md5Hashes: string[];
}
